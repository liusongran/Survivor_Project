...
//#include <profile.h.t#include <profile.h.temp>
...
static uint16_t __elk_crc(uint16_t start, uint16_t end, uint8_t bufIdx, uint16_t paddings) {
...
PRB_START(crcPrf)
    ...
    for(tempI=0;tempI<tempSize;tempI=tempI+2){
        HWREG16(CRC_BASE + OFS_CRCDI) = HWREG16(bufAddrStart+start+tempI);
